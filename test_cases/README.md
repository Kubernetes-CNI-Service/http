...
选到不能生成 MLAG 配置的模板也会在发布前失败。测试还覆盖 MLAG `peerlink` 显式复制
`br_default` 的全部业务 VLAN selector：接口侧只生成空 mapping，不复制全局 VNI 属性；展开后
两侧 VLAN 集合必须完全一致，且 VLAN 4094 只能用于独立的 `peerlink.4094` 子接口。
